...
void ximopen(Display *dpy) {
    ...
    xw.xic = XCreateIC(xw.xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow,
...
void xinit(int cols, int rows) {
    ...
    xw.win = XCreateWindow(
        xw.dpy, parent, xw.l, xw.t, win.w, win.h, 0, xw.depth, InputOutput, xw.vis,
    ...
    if (file) {
        ...
        XChangeProperty(xw.dpy, xw.win, xw.netwmicon, XA_CARDINAL, 32, PropModeReplace,
...
void xrdb_load(void) {
    ...
    if (xrm != NULL) {
        ...
        /* cursorblinkstate = 1; // in case if cursor shape was changed from a blinking one to a
         * non-blinking */
