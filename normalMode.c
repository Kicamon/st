...
static ExitState expandExpression(char l) { //    ({ =>)       l  ?  {  \n | l | v  /  } \n | h | y
    ...
    if (lc == 'w')
        mot[2] = (char)('b' - lc + l), mot[7] = (char)((a ? 'w' : 'e') - lc + l),
        mot[9] = (char)(a ? 'h' : 0);
    else {
        mot[1] = '?', mot[3] = mot[8] = '\n', mot[6] = '/', mot[4] = (char)(a ? 0 : 'l'),
        mot[9] = (char)(a ? 0 : 'h');
        for (int i = found = 0; !found && i < 6; ++i)
            if ((found = contains(l, braces[i], 2)))
                mot[2] = braces[i][0], mot[7] = braces[i][1];
    }
    if (!found)
        return failed;
    assign(&lCmd, &cCmd);
...
ExitState kPressHist(char const *cs, size_t len, int ctrl, KeySym const *kSym) {
    ...
    int const prevYOff = IS_SET(MODE_ALTSCREEN) ? 0 : histOff,
              search = state.m.search && state.m.active, prevAltToggle = altToggle,
              prevOverlay = overlay;
    int const noOp = !state.cmd.op && !state.cmd.infix, num = len == 1 && BETWEEN(cs[0], 48, 57),
              esc = kSym && *kSym == XK_Escape,
              ret = (kSym && *kSym == XK_Return) || (len == 1 && cs[0] == '\n'),
              quantifier = num && (cs[0] != '0' || state.m.c),
              ins = !search && noOp && len && cs[0] == 'i';
    exited = 0;
    ExitState result = success;
    if (esc || ret || ins) {
        result = exitMotion, len = 0;
    } else if (kSym && *kSym == XK_BackSpace) {
        if ((search || state.m.c) && size(&cCmd))
            pop(&cCmd);
        if (search) {
            if (size(&searchStr))
                pop(&searchStr);
    ...
    styleCmd = style[state.cmd.op == yank
                         ? 1
                         : (state.cmd.op == visual ? 2 : (state.cmd.op == visualLine ? 3 : 0))];
...
void historyOverlay(int x, int y, Glyph *g) {
    ...
    if (overlay && term.col > 9 && term.row > 4 && (x > (2 * term.col / 3)) &&
        (y >= (term.row - 2))) {
        *g = (y == term.row - 2) ? styleSearch : styleCmd;
        if (y == term.row - 2)
            ...
            getChar(size(&cCmd) ? &cCmd : &lCmd, g, term.row - 1, term.col - 7, term.col / 3 - 6,
                    x);
